...
CSOURCES+=$(shell find -L $(INCDIR) -name '*.c')
...
ifeq ($(wildcard /opt/openocd/bin/openocd),)
	 /usr/bin/openocd -f /usr/share/openocd/scripts/board/stm32f4discovery.cfg -c "program bin/"$(BINHEX)"  verify reset"
else
	@echo hi! /opt/openocd/bin/openocd -f /opt/openocd/share/openocd/scripts/board/stm32f4discovery.cfg -c "program bin/"$(BINHEX)" verify reset"
endif
